...
const s = {
  ...
  input: { width: '100%', padding: '10px 14px', fontSize: 15, border: '1px solid #333', borderRadius: 10, background: '#111', color: '#fff', outline: 'none' },
...
function NavTab({ label, icon, active, onClick }) {
  return <button onClick={onClick} style={{ padding: '6px 14px', borderRadius: 8, fontSize: 13, cursor: 'pointer', border: active ? '1px solid #444' : '1px solid transparent', background: active ? '#2a2a2a' : 'transparent', color: active ? '#fff' : '#888', fontWeight: active ? 600 : 400 }}>{icon} {label}</button>;
}
...
export default function App() {
  ...
  const loadBusiness = useCallback(async () => {
    const { data } = await supabase.from('businesses').select('*').eq('slug', BUSINESS_SLUG).single();
    if (data) setBusiness(data);
    return data;
  }, []);

  const loadHorario = useCallback(async (businessId) => {
    const { data } = await supabase.from('schedule_config').select('*').eq('business_id', businessId);
    if (data) {
      const h = {};
      data.forEach(row => { h[row.day_of_week] = { active: row.active, open: row.open_time, close: row.close_time }; });
      setHorario(h);
    }
  }, []);

  const loadCitas = useCallback(async (businessId) => {
    const { data } = await supabase.from('appointments').select('*').eq('business_id', businessId).neq('status', 'cancelled').order('date', { ascending: true }).order('time', { ascending: true });
  ...
  async function confirmar(id) {
    ...
    showToast('Confirmada — WhatsApp a ' + cita.client_name.split(' ')[0]);
    if (business) loadCitas(business.id);
  }
  ...
  async function saveHorario() {
    ...
    for (let i = 0; i < 7; i++) {
      const h = horario[i];
      if (!h) continue;
      await supabase.from('schedule_config').update({ active: h.active, open_time: h.open, close_time: h.close }).eq('business_id', business.id).eq('day_of_week', i);
    }
    showToast('Horario guardado');
  }

  const pendientes = citas.filter(c => c.status === 'pending');
  const confirmadas = citas.filter(c => c.status === 'confirmed');
  const byDate = {};
        ...
        <div style={{ ...s.page, maxWidth: 480, margin: '0 auto' }}>
          ...
          {step === 2 && (
            <>
              <div style={s.card}>
                ...
                <div style={s.calGrid}>{DAYS.map(d => <div key={d} style={s.calHeader}>{d}</div>)}</div>
                <div style={s.calGrid}>
          ...
          {step === 5 && (
            <div style={{ textAlign: 'center', paddingTop: '1rem' }}>
              ...
              <p style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>Recibirás un WhatsApp cuando se confirme</p>
        ...
        <div style={s.page}>
              ...
              {pendientes.length === 0
                ...
                : pendientes.map(c => (
                  <div key={c.id} style={s.citaCard}>
                    <div style={s.avatar}>{getInitials(c.client_name)}</div>
          ...
          {panel === 'horario' && (
            <div>
              ...
              {DAYS_FULL.map((day, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 1rem', background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 10, marginBottom: 6 }}>
                  ...
                  <label style={{ position: 'relative', width: 36, height: 20, flexShrink: 0 }}>
                    <input type="checkbox" checked={horario[i]?.active || false} onChange={e => setHorario(h => ({ ...h, [i]: { ...h[i], active: e.target.checked } }))} style={{ opacity: 0, width: 0, height: 0 }} />
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, opacity: horario[i]?.active ? 1 : 0.3 }}>
                    <input type="time" value={horario[i]?.open || '10:00'} disabled={!horario[i]?.active} onChange={e => setHorario(h => ({ ...h, [i]: { ...h[i], open: e.target.value } }))} style={{ padding: '4px 8px', fontSize: 13, border: '1px solid #333', borderRadius: 8, background: '#111', color: '#ccc', width: 80 }} />
                    <span style={{ color: '#555' }}>–</span>
                    <input type="time" value={horario[i]?.close || '21:00'} disabled={!horario[i]?.active} onChange={e => setHorario(h => ({ ...h, [i]: { ...h[i], close: e.target.value } }))} style={{ padding: '4px 8px', fontSize: 13, border: '1px solid #333', borderRadius: 8, background: '#111', color: '#ccc', width: 80 }} />
                  </div>
                </div>
              ))}
              <button onClick={saveHorario} style={{ ...s.btnGold, marginTop: '1rem' }}>Guardar horario</button>
            </div>
          )}
        </div>
      )}
